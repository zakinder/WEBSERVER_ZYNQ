...
architecture arch_imp of videoProcess_v1_0_m_axis_mm2s is
    ...
    signal axis_tdata               : std_logic_vector(s_data_width-1 downto 0):= (others => '0'); 
    signal maxis_mm2s_tdata         : std_logic_vector(s_data_width-1 downto 0):= (others => '0');
...
process (aclk) begin
    if (rising_edge (aclk)) then
        ...
        when wait_to_go =>
            wrchx_cont         <= 0;
            axis_tlast         <='0';
            rgb_s_axis_tready  <='1';--initiate
            maxis_mm2s_tvalid  <='0';
            if (rgb_s_axis_tvalid  = '1')then-- initiate response
